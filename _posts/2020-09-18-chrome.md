...
**重排和重绘都是运行在主线程上. 而 js 也是运行在主线程上.**(联系 event loop) 于是就会出现抢占执行时间的问题. 如果你写了一个不断导致重排重绘的动画, 浏览器则需要在每一帧(frame)都运行样式计算布局和绘制的操作. 
