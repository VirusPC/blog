...
[上一篇](/blog/机器学习/2019/04/12/optimization.html)讲过，与普通的批量梯度下降下降的相比，小批量梯度下降在一次迭代中可以进行多次梯度更新，使得更新速度快于批量梯度下降；但是它每次下降的方向却大多是偏离正确方向的。回想刚刚提到的指数加权平均，它以当前值为主，并且兼顾之前的值，将它应用于小批量梯度下降就会使得每次梯度更新时，不但考虑了本次的数据，还考虑到了之前的数据，使得梯度下降的方向得到了一定程度上的矫正。直观上来说就是通过取平均值，使得代价函数的值的曲线变得更直，更加平滑了。
